...
def orders(request: HttpRequest) -> HttpResponse:
    logger.debug(f"Orders list requested: user_id={request.user.id}")

    latest_statuses = get_latest_order_statuses()

    form = OrderFilterForm(request.GET)
    if form.is_valid():
        logger.debug(f"Orders filter applied: {form.cleaned_data}")
        latest_statuses = filter_orders_status(latest_statuses, form.cleaned_data)

    page = get_paginator_page(request, latest_statuses)

    logger.info(
        f"Orders page loaded: user_id={request.user.id}, count={page.paginator.count}"
    )
...
def order_info(request: AuthenticatedRequest, order_code: int) -> HttpResponse:
    logger.debug(
        f"Order info opened: user_id={request.user.id}, order_code={order_code}"
    )
    ...
    if request.method == "POST":
        logger.info(
            f"Order status update attempt: user_id={request.user.id},"
            f" order_id={order.id}"
        )
        ...
        if form.is_valid():
            ...
            logger.info(
                f"Order status updated: order_id={order.id}, user_id={request.user.id}"
            )
        ...
        logger.warning(
            f"Invalid order status form: order_id={order.id}, errors={form.errors}"
        )
